...
Computing DMRs:
```{r warning=FALSE, message=FALSE, eval=FALSE}
cmd <- paste("./", methPipe.path, "/dmr ", resDir, "/neg_control_methpipe.methdiff ", resDir, "/neg_control_grp1.hmr ", resDir, "/neg_control_grp2.hmr ", resDir, "/neg_control_methpipe_dmrs_grp1 ", resDir, "/neg_control_meth_dmrs_grp2", sep="")
system(cmd)
```
...
cmd <- paste("awk -F '[:\t]' '$5 >= 10 && $6 >= 5 {print $0}' ", resDir, "/neg_control_methpipe_dmrs_grp2 > ", resDir, "/neg_control_methpipe_dmrs_grp2
             .filtered", sep="")
system(cmd)
```
