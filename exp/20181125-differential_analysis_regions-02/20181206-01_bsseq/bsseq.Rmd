...
bs.nc <- read.bismark(files = files.nc, sampleNames = pheno.nc$names, strandCollapse = F, 
...
bs.sim <- read.bismark(files = files.sim, sampleNames = pheno.sim$names, strandCollapse = F, 
                        mc.cores = detectCores() - 1)
...
```{r}
bs.tstat.nc <- BSmooth.tstat(bs.smoothed.nc, mc.cores = detectCores() -1, estimate.var = "same",
                             group1 = pheno.nc$names[pheno.nc$Group == "Group1"],
                             group2 = pheno.nc$names[pheno.nc$Group == "Group2"],
                             local.correct = TRUE,
                             verbose = TRUE)

bs.tstat.sim <- BSmooth.tstat(bs.smoothed.sim, mc.cores = detectCores() -1, estimate.var = "same",
...
dmr.nc$pValue <- pt(abs(dmr.nc$areaStat), 4, lower.tail = FALSE)
dmr.nc$qValue.BH <- p.adjust(p = dmr.nc$pValue, method = "BH")
dmr.nc <- subset(dmr.nc, n >= 10 & abs(meanDiff) >= 0.1)
write.table(dmr.nc, file = gzfile("bsseq_dmr_neg_control.txt.gz"), quote = F, row.names = F)

# Simulated data
dmr.sim <- dmrFinder(bs.tstat.sim)
dmr.sim$pValue <- pt(abs(dmr.sim$areaStat), 4, lower.tail = FALSE)
dmr.sim$qValue.BH <- p.adjust(p = dmr.sim$pValue, method = "BH")
dmr.sim <- subset(dmr.sim, n >= 10 & abs(meanDiff) >= 0.1)
write.table(dmr.sim, file = gzfile("bsseq_dmr_simulated_data.txt.gz"), quote = F, row.names = F)
```
...
## Negative control
```{r}
c1 <- dmr.nc$qValue.BH[dmr.nc$qValue.BH <= 0.05]
datatable(
  dmr.nc,
  rownames = F,
...
## Simulated data
```{r}
c2 <- dmr.sim$qValue.BH[dmr.sim$qValue.BH <= 0.05]
datatable(
  dmr.sim,
  rownames = F,
  filter = "top", extensions = c("Buttons", "ColReorder"), options = list(
