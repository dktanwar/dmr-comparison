...
dmr.nc$qval.BH <- p.adjust(p = dmr.nc$pVal, method = "BH")
dmr.nc <- dmr.nc[order(dmr.nc$qval.BH), ]
write.table(dmr.nc, file = gzfile("./output/cgmaptools_dmr_neg_control.txt.gz"), sep = "\t", quote = F, row.names = F)
```

```{r}
c1 <- dmr.nc$qval.BH[dmr.nc$qval.BH <= 0.05]
datatable(
  dmr.nc,
  rownames = F,
...
dmr.sim$qval.BH <- p.adjust(p = dmr.sim$pVal, method = "BH")
dmr.sim <- dmr.sim[order(dmr.sim$qval.BH), ]
