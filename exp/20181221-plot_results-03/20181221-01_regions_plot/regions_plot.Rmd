...
colnames(bs.sim$bs) <- anno.sim$names
```
...
## NC
```{r}
files.nc <- list.files(path = "./input", pattern = "neg_control.txt|negative_control", full.names = T)
names(files.nc) <- sapply(files.nc, function(x) strsplit(x, "\\.|/")[[1]][4])

read.nc <- lapply(files.nc, function(x) GRanges(read.table(x, sep = "\t", stringsAsFactors = F, header = T)))
...
files.sim <- list.files(path = "./input", pattern = "sim_data.txt|simulated_data.txt", full.names = T)
...
plotDMRs(BSseq = bs.sim$bs, regions=bs.sim$gr.dmrs[1:10,], testCovariate="Group",
    compareTrack = GenomicRangesList(read.sim[c(1:4)]), qval = F, stat = F)
```
